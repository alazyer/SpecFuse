/**
 * SpecFuse CI core seam.
 *
 * Business logic for the CI integration (drift normalization, exit-code
 * calculation, history recording, GitHub Actions workflow generation) lives
 * here so it can be shared by both the CLI command layer
 * (`src/commands/ci.js`) and the programmatic API (`src/api/ci.mjs`)
 * without the API importing from the command layer.
 *
 * This module is the core→API layering seam: it returns structured data
 * and never calls `process.exit` or writes to stdout. Presentation
 * (formatting) is delegated to `src/core/ci-output.js`; CLI presentation
 * and `process.exit` wiring belong to the command layer.
 */
...
import { formatAuto } from './ci-output.js'
...
export async function ciInit(projectRoot, options = {}) {
  // Default to specfuse-ci.yml to match docs/ci-integration.md
  const defaultPath = join(projectRoot, '.github', 'workflows', 'specfuse-ci.yml')
  ...
  if (options.github === false) {
    throw new CiUnsupportedModeError(
      'Only GitHub Actions workflow is supported. Use --github (default) to generate.',
      {
        supportedMode: 'github',
...
// Re-export the history event types so consumers of the core seam have access
// to the same constants without reaching into a different module.
export { EVENT_TYPES }
