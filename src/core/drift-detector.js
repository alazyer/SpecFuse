...
/**
 * Resolve path to constitution.md (always at project root in v3).
...
async function checkSingleRuleDrift(projectRoot, registry, rule) {
  ...
  const sourceContent = sourceIsDir
    ? `dir:${rule.source}`
    : await readFileSafe(sourcePath);

  if (!sourceContent && !pathExists(sourcePath)) {
    return { ruleId, state: 'SOURCE_MISSING', sourceId, targetId,
  ...
  if (!lastSync) return { ruleId, state: 'NEVER_SYNCED', sourceId, targetId,
  ...
  if (srcChanged && tgtChanged) return { ruleId, state: 'BOTH_CHANGED', sourceId, targetId,
  ...
  if (srcChanged) return { ruleId, state: 'SOURCE_CHANGED', sourceId, targetId,
    message: `${rule.source} changed — [${rule.section}] is stale.`,
    remediation: 'Run `specfuse sync`.' };
  if (tgtChanged) return { ruleId, state: 'TARGET_CHANGED', sourceId, targetId,
  ...
  return { ruleId, state: 'IN_SYNC', sourceId, targetId,
    message: `[${rule.section}] in ${rule.target} is current.`, remediation: '' };
}
...
async function checkMultiTargetDrift(projectRoot, registry, rule) {
  ...
  if (!constitutionContent) {
    return [{ ruleId: rule.id, state: 'SOURCE_MISSING',
      sourceId: rule.source, targetId: rule.target,
  ...
  return Promise.all(changeDirs.map(async (entry) => {
    ...
    if (!lastSync) return { ruleId, state: 'NEVER_SYNCED',
      sourceId: rule.source, targetId,
    ...
    if (!srcChanged && !tgtChanged) return { ruleId, state: 'IN_SYNC',
      sourceId: rule.source, targetId,
      message: `${changeName}: constitutional header is current.`, remediation: '' };

    return { ruleId,
      state: srcChanged && tgtChanged ? 'BOTH_CHANGED'
           : srcChanged ? 'SOURCE_CHANGED' : 'TARGET_CHANGED',
      sourceId: rule.source, targetId,
