...
export function getChangeTitle(proposalContent = '', fallback = 'Untitled Change') {
  return proposalContent.match(/^#\s+Change Proposal:\s+(.+)$/m)?.[1]
    ?? proposalContent.match(/^#\s+(.+)$/m)?.[1]
    ?? fallback;
}

export function extractAcceptanceCriteria(markdown = '') {
  const section = extractH2SectionAny(markdown, ['Acceptance Criteria', 'AC', 'Done When', 'Criteria']);
  if (!section) return [];
...
export function countChecklist(markdown = '', pattern = /^- \[[ xX]\]/gm) {
  ...
  return { total: matches.length, checked: checked.length, remaining: matches.length - checked.length };
}
...
export function detectUiImpact(designContent = '') {
  const match = designContent.match(/^\*\*Affects UI:\*\*\s*(.+)$/mi);
...
export function getChangeProposalState(proposalContent = '', options = {}) {
  ...
  const reviewStatus = normalizeReviewStatus(parseFrontmatterDocument(options.reviewContent ?? '').data?.status);
  const verifyStatus = normalizeVerifyStatus(parseFrontmatterDocument(options.verifyContent ?? '').data?.status);
