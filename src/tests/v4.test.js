...
async function setupChange(root, { proposal = false, archived = false } = {}) {
  ...
  if (archived) {
    await mkdir(join(root, '.specfuse', 'changes', 'archive', '2026-04-01-user-auth'), {
      recursive: true,
    })
    await writeFile(
...
describe('Rule loader', () => {
  ...
  test('loads all 6 built-in rules', async () => {
    ...
    assert.ok(
      rules.some((r) => r.id.includes('design-system')),
      'design system rule must be loaded',
    )
  })
  ...
  test('all source paths are under .specfuse/ or are .specfuse/constitution.md', async () => {
    ...
    for (const r of rules) {
      const validSource =
        r.source.startsWith('.specfuse/') || r.source === '.specfuse/constitution.md'
      assert.ok(
        validSource,
  ...
  test('Pass A rules feed into constitution, Pass B flows out', async () => {
    ...
    assert.ok(
      passA.every((r) => r.target === '.specfuse/constitution.md'),
      'All Pass A rules should target constitution.md',
    )
    assert.ok(
      passB.every((r) => r.source === '.specfuse/constitution.md'),
  ...
  test('constitution→changes rule resolves change directories not flat files', async () => {
    ...
    assert.ok(
      targets.every((t) => t.endsWith('proposal.md')),
      'each target must be proposal.md inside a change directory',
    )
    assert.ok(
      !targets.some((t) => t.includes('archive')),
...
describe('Two-pass sync engine', () => {
  ...
  test('Pass A syncs arch decisions into constitution, Pass B injects header into proposal', async () => {
    ...
    assert.ok(
      passA.some((r) => r.changed && r.ruleId.includes('arch')),
      'Pass A should sync architecture decisions',
    )
    assert.ok(
      passB.some((r) => r.changed),
  ...
  test('Pass B is skipped when Pass A has an error', async () => {
    ...
    const rules = await loadRules(root)
    const brokenRule = {
      id: 'broken→constitution:broken',
      pass: 'A',
      source: '.specfuse/plan/broken.md',
  ...
  test('registry persists after sync', async () => {
    ...
    await fresh.load()
    const lastSync = fresh.getLastSync(
...
describe('Registry v4', () => {
  ...
  test('all artifact paths are under .specfuse/ or project root', () => {
    ...
    for (const [id, desc] of Object.entries(defaults)) {
      const isInternal =
        desc.path.startsWith('.specfuse/') || desc.path === '.specfuse/constitution.md'
      assert.ok(
        isInternal,
...
describe('Change commands v4', () => {
  ...
  test('change review and verify generate artifacts from proposal and constitution', async () => {
    ...
    const review = await readFile(
      join(root, '.specfuse', 'changes', 'add-cart', 'review.md'),
      'utf8',
    )
  ...
  test('change archive blocks when verify.md is missing or unverified', async () => {
    ...
    await assert.rejects(() => changeArchive(root, 'add-cart'), /EXIT:1/)
  })
...
describe('CLI integration v4', () => {
  ...
  test('change new applies custom schema instructions to generated artifacts', async () => {
    await writeFile(
      join(root, '.specfuse', 'artifact-schema.json'),
      JSON.stringify(
        {
          version: 1,
          artifacts: {
