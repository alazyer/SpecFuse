...
export async function driftCommand(projectRoot, options = {}) {
  ...
  let driftCount = 0, syncedCount = 0, missingCount = 0;
  ...
  for (const r of results) {
    ...
    if (r.state !== 'IN_SYNC' && r.state !== 'SOURCE_MISSING') {
      ...
      driftCount++;
    } else if (r.state === 'SOURCE_MISSING') { missingCount++; }
    else { syncedCount++; }
    logger.br();
  }
  ...
  if (driftCount > 0) {
    ...
    logger.info(`Run ${chalk.cyan('specfuse sync')} to resolve, or ${chalk.cyan('specfuse diff')} to preview changes.`);
