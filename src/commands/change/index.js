...
function fillReviewTemplate(template, vars) {
  return Object.entries(vars).reduce((content, [key, value]) => content.replaceAll(`{{${key}}}`, value), template);
}
...
async function updateProposalStatus(proposalPath, proposalContent, updates = {}) {
  ...
  const next = `---\n${Object.entries(data).map(([key, value]) => `${key}: ${value == null ? '~' : value}`).join('\n')}\n---\n\n${parsed.content.trimStart()}`;
...
export async function changeNew(projectRoot, name) {
  ...
  logger.info(`Run ${chalk.cyan(`specfuse change review ${slug}`)} and ${chalk.cyan(`specfuse change verify ${slug}`)} once the change is ready for review and verification`);
...
export async function changeList(projectRoot) {
  ...
  if (!activeEntries.length) {
    ...
    for (const entry of activeEntries) {
      ...
      const { proposal, design, review, verify } = await readChangeFiles(join(changesDir, entry.name));
      const title = getChangeTitle(proposal, entry.name);
      const status = getChangeProposalState(proposal, { reviewContent: review, verifyContent: verify });
      ...
      const bar    = ac.length ? chalk.dim(`${verifyProgress.checked}/${verifyProgress.total || ac.length} verified`) : chalk.dim('0 AC');
      const uiImpact = detectUiImpact(design);
      const reviewStatus = review ? reviewStatusLabel(review) : 'missing';
      console.log(`  ${chalk.green('◦')}  ${chalk.bold(title)}  ${chalk.dim(entry.name)}  ${chalk.dim(`[${status}]`)}  ${bar}  ${chalk.dim(`review:${reviewStatus}`)}  ${chalk.dim(`ui:${uiImpact}`)}  ${chalk.dim(mtime?.toISOString().slice(0,10) ?? '')}`);
    }
  }
  ...
  if (!archivedEntries.length) {
    ...
    for (const entry of archivedEntries) {
      ...
      console.log(`  ${chalk.dim('✔')}  ${chalk.dim(title)}  ${chalk.dim(entry.name)}  ${chalk.dim(`[${verified === 'pass' ? 'verified' : 'unverified'}]`)}`);
    }
...
export async function changeShow(projectRoot, name) {
  ...
  logger.row('Status', getChangeProposalState(content, { archived: resolved.archived, reviewContent, verifyContent }), chalk.yellowBright);
  logger.row('Review', reviewContent ? reviewStatusLabel(reviewContent) : 'not generated', reviewContent ? chalk.cyan : chalk.dim);
  const verifySummary = summarizeVerifyProgress(verifyContent);
  logger.row('Verify', verifyContent ? `${verifySummary.status} (${verifySummary.checked}/${verifySummary.total} confirmed)` : 'not generated', verifyContent ? chalk.cyan : chalk.dim);
  logger.br();
...
export async function changeReview(projectRoot, name) {
  ...
  logger.br();
  if (created) logger.success(`Generated ${chalk.cyan(`${resolved.archived ? `.specfuse/changes/archive/${resolved.archiveName}` : `.specfuse/changes/${resolved.slug}`}/review.md`)}`);
...
export async function changeVerify(projectRoot, name) {
  ...
  logger.br();
  if (created) logger.success(`Generated ${chalk.cyan(`${resolved.archived ? `.specfuse/changes/archive/${resolved.archiveName}` : `.specfuse/changes/${resolved.slug}`}/verify.md`)}`);
  ...
  if (progress.remaining > 0) logger.warn(`${progress.remaining} acceptance criteria still need confirmation before archival.`);
  logger.br();
}
...
export async function changeArchive(projectRoot, name, options = {}) {
  ...
  if (verifyStatus !== 'pass' && !options.force) {
    ...
    if (!verifyContent) {
      ...
      logger.row('Confirmed AC', `${verifyProgress.checked}/${verifyProgress.total}`, chalk.yellowBright);
    }
    logger.warn(`Use ${chalk.cyan(`specfuse change archive ${slug} --force`)} to override and archive as unverified.`);
    process.exit(1);
  }
  ...
  if (verifyStatus !== 'pass') logger.warn('Archived with unverified acceptance criteria (--force used).');
