...
export async function initCommand(projectRoot, options = {}) {
  logger.header('SpecFuse Init  v3');
  logger.br();
  ...
  console.log(chalk.dim([
    ...
    '  ├── plan/             ← planning artifacts (PRD, architecture, stories)',
  ...
  if (phase === 'planning' || phase === 'unknown') {
    ...
    logger.info(`3. ${chalk.cyan('specfuse specify init')}     Generate constitution.md from plan`);
    logger.info(`4. ${chalk.cyan('specfuse sync')}             Sync all artifacts`);
