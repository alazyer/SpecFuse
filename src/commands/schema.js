...
export async function schemaInitCommand(projectRoot, options = {}) {
  ...
  logger.info(
    `Edit ${chalk.cyan(result.displayPath)} to customize instructions before generating artifacts.`,
  )
  logger.br()
}
...
export async function schemaShowCommand(projectRoot, options = {}) {
  ...
  logger.row(
    'Status',
    schema.exists ? 'Found' : 'Not found',
    schema.exists ? chalk.green : chalk.yellow,
  )
  ...
  for (const [artifactId, instructions] of entries) {
    logger.row(
      ...
      instructions.length ? chalk.cyan : chalk.dim,
    )
  }
  logger.br()
}
