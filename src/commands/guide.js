...
function parsePersona(persona) {
  const raw = String(persona ?? 'new-user').trim().toLowerCase();
...
async function collectGuideState(projectRoot) {
  ...
  const activeChangeNames = (await listDirectories(join(projectRoot, '.specfuse', 'changes')))
    .filter(name => name !== 'archive');
  const archivedChanges = await countDirectories(join(projectRoot, '.specfuse', 'changes', 'archive'));
...
function buildGuidance(state, persona) {
    ...
    return {
      ...
      steps: [
        ...
        },
        {
          ...
          reason: 'Create one shared rules file from your plan, then apply those rules across your spec files.',
        },
      ],
  ...
  if (effectivePhase === 'planning' || !state.hasConstitution) {
    ...
    if (!state.hasDesignSystem) addStep(steps, 'specfuse plan design system', 'Create reusable design constraints for UI consistency.');
    if (state.storyCount === 0) addStep(steps, 'specfuse plan story "First story"', 'Seed user stories so constitutional user-story sections stay meaningful.');
  ...
  if (effectivePhase === 'feature-dev') {
    ...
    if (persona === 'developer') addStep(steps, 'specfuse watch', 'Keep sync automatic while editing planning or change artifacts.');
    if (persona === 'qa') addStep(steps, 'specfuse drift --fail', 'Use CI-friendly drift checks to block stale spec state.');
    return { focus, steps, effectivePhase };
  }

  if (state.activeChanges === 0) removeStep(steps, 'specfuse change list');
  if (persona === 'qa') addStep(steps, 'specfuse doctor', 'Run diagnostics to catch malformed or incomplete artifacts.');
  return { focus, steps, effectivePhase };
}
...
export async function guideCommand(projectRoot, options = {}) {
  ...
  const phaseLabel = guidance.effectivePhase === state.phase
    ? state.phase
  ...
  const warnings = [];
  if (!personaValid) warnings.push(`Unknown persona '${rawPersona}'. Using 'new-user'. Allowed: new-user, planner, developer, qa.`);
  if (!state.isGitRepo) warnings.push('No .git directory found at --root. If you are in a subdirectory, rerun with --root <repository-root>.');
  ...
  logger.row('Active changes', String(state.activeChanges), state.activeChanges ? chalk.cyan : chalk.dim);
  logger.row('Archived changes', String(state.archivedChanges), state.archivedChanges ? chalk.green : chalk.dim);
  logger.br();
