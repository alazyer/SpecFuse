...
export async function planPrd(projectRoot, options = {}) {
  ...
  logger.info(
    `3. Run ${chalk.cyan('specfuse sync')} to propagate decisions to .specfuse/constitution.md`,
  )
  logger.br()
}
...
export async function planArch(projectRoot, options = {}) {
  ...
  if (exists) {
    logger.info(
      `Architecture doc already exists at ${chalk.cyan('.specfuse/plan/architecture.md')}`,
    )
  ...
  logger.info(
    `2. Run ${chalk.cyan('specfuse sync')} to populate .specfuse/constitution.md [plan-decisions]`,
  )
  logger.br()
}
...
export async function planStory(projectRoot, title, options = {}) {
  ...
  logger.info(
    `Run ${chalk.cyan('specfuse sync')} to propagate stories to .specfuse/constitution.md [user-stories]`,
  )
  logger.br()
}
...
export async function planList(projectRoot) {
  ...
  console.log(
    `  ${prdExists ? chalk.green('✔') : chalk.dim('○')}  ${chalk.bold('PRD')}  ${chalk.dim('.specfuse/plan/prd.md')}  ${chalk.dim(prdTime?.toISOString().slice(0, 10) ?? 'not created')}`,
  )
  ...
  console.log(
    `  ${archExists ? chalk.green('✔') : chalk.dim('○')}  ${chalk.bold('Architecture')}  ${chalk.dim('.specfuse/plan/architecture.md')}  ${chalk.dim(archTime?.toISOString().slice(0, 10) ?? 'not created')}`,
  )
  ...
  console.log(
    `  ${systemExists ? chalk.green('✔') : chalk.dim('○')}  ${chalk.bold('Design System')}  ${chalk.dim('.specfuse/plan/design/system.md')}  ${chalk.dim(systemTime?.toISOString().slice(0, 10) ?? 'not created')}`,
  )
  ...
  for (const [label, subdir, commandHint] of [
    ...
    console.log(
      `  ${count > 0 ? chalk.green('✔') : chalk.dim('○')}  ${chalk.bold(label)}  ${chalk.dim(`.specfuse/plan/design/${subdir}/`)}  ${chalk.dim(`${count} file(s)`)}`,
    )
  ...
  if (!pathExists(storiesDir)) {
    ...
    if (!files.length) {
      ...
      for (const file of files.sort()) {
        ...
        console.log(
          `  ${chalk.green('◦')}  ${chalk.bold(title)}  ${chalk.dim(file)}  ${chalk.dim(bar)}`,
        )
      }
    }
  }
...
export async function planDesignSystem(projectRoot, options = {}) {
  ...
  logger.br()
  logger.info(
    `Edit ${chalk.cyan('.specfuse/plan/design/system.md')} — capture design tokens, accessibility rules, layout constraints, and component standards.`,
  )
  logger.info(
    `Run ${chalk.cyan('specfuse sync')} to propagate design constraints into .specfuse/constitution.md`,
  )
  logger.br()
}
...
export async function planDesignScreen(projectRoot, title, options = {}) {
  ...
  const { nextNum, filename } = await nextNumberedFilename(
    screensDir,
    'screen',
    title,
    'new-screen',
...
export async function planDesignList(projectRoot) {
  ...
  const systemTime = systemExists ? await getModifiedTime(systemPath) : null
  console.log(
    `  ${systemExists ? chalk.green('✔') : chalk.dim('○')}  ${chalk.bold('System')}  ${chalk.dim('.specfuse/plan/design/system.md')}  ${chalk.dim(systemTime?.toISOString().slice(0, 10) ?? 'not created')}`,
  )
  if (!systemExists) console.log(`     ${chalk.dim('→ specfuse plan design system')}`)
